...
namespace Movies.Presentation.Modules
{
    public static class MoviesModule
    {
        ...
        {
            app.MapGet("/api/movies", async (IMediator mediator, [FromQuery] int pageSize, 
                [FromQuery] int pageNumber,CancellationToken ct) =>
            {
                var movies = await mediator.Send(new GetMoviesQuery(new Contracts.Requests.PaginationParams
                {
                    PageNumber = pageNumber,
                    PageSize = pageSize
                }), ct);

                return Results.Extensions.OkPaginationResult(movies.PageSize, movies.CurrentPage, 
                    movies.TotalCount, movies.TotalPages, movies.Items);
            }).WithTags("Movies");
